...
def image_to_base64(img_path):
    try:
        ...
        """I used base64 because Streamlit and HTML often embed images in base64 when they can’t load from file paths directly."""
...
def load_known_faces():
    ...
    for person in os.listdir(
        KNOWN_FACES_DIR
    ):  # it loops through each folder in the known_faces directory
        person_dir = os.path.join(
            KNOWN_FACES_DIR, person
        )  # person_dir: The full path to the folder for one person's images.
        if not os.path.isdir(person_dir):
            continue

        for img_name in os.listdir(
            person_dir
        ):  # Loops through every image inside each person's folder.
            ...
            try:
                ...
                if encodings:
                    known_faces_encodings.append(
                        encodings[0]
                    )  # We take the first encoding found in the image
                    known_faces_names.append(
                        person
                    )  # Associate the encoding with the person's name
            except Exception as e:
                st.warning(f"Could not process image {img_path}: {e}")

    if not known_faces_encodings:
        st.warning(
            "Warning: No known faces were loaded. Please check the 'known_faces' directory."
        )
...
def main():
    ...
    # --- Display Header ---
    eye_logo_b64 = image_to_base64(
        "Gemini_Generated_Image_pit6rspit6rspit6-removebg-preview.png"
    )
    ...
    def recognize_faces_in_frame(frame):
        ...
        results = detector(
            frame, verbose=False
        )  # Runs the face detector model on the input frame. and "verbose=False" prevents detailed output
        boxes = (
        ...
        for box in boxes:
            ...
            ]  # Convert coordinates to integers (x1, y1) is the top-left corner, (x2, y2) is the bottom-right corner
            face = frame[
                y1:y2, x1:x2
            ...
            rgb_face = cv2.cvtColor(
                face, cv2.COLOR_BGR2RGB
            )  # Convert the cropped face to RGB format to be compatible with face_recognition
            encodings = face_recognition.face_encodings(
                rgb_face
            )  # Get the face encodings for the cropped face

            name = "Unknown"  # Default name
            if encodings:
                emb = encodings[0]  # We take the first encoding found
                matches = face_recognition.compare_faces(
                    known_faces, emb, tolerance=0.5
                )  # Compare the encoding with known faces
                face_distances = face_recognition.face_distance(
                    known_faces, emb
                )  # Get distances to known faces and the smallest distance indicates the best match
                if True in matches:
            ...
            cv2.putText(
                ...
                (255, 255, 255),
                2,
            )  # Put the name label above the box
    ...
    if option == "Image":
        ...
        if uploaded_image:
            img = Image.open(uploaded_image).convert(
                "RGB"
            )  # Convert to RGB because OpenCV uses BGR by default
            img_array = np.array(img)
            frame_bgr = cv2.cvtColor(
                img_array, cv2.COLOR_RGB2BGR
            )  # Convert RGB to BGR for OpenCV
            processed_frame, attendees = recognize_faces_in_frame(
                frame_bgr
            )  # Process the image
            st.image(
        ...
        if uploaded_video:
            ...
            while cap.isOpened():
                ...
                if confirmed_attendees:
                    status_text.success(
                        f"Confirmed Attendees: {', '.join(sorted(list(confirmed_attendees)))}"
                    )
                else:
            ...
            if confirmed_attendees:
                st.success(
                    f"✅ Final Attendees Detected: {', '.join(sorted(list(confirmed_attendees)))}"
                )
            else:
                st.warning(
                    "⏹️ Processing complete. No known attendees were identified in the video."
                )
    ...
    # --- Handle Webcam ---
        ...
        if camera:
            ...
            st.image(
                cv2.cvtColor(processed_frame, cv2.COLOR_BGR2RGB),
                caption="Processed Webcam Frame",
            )
            st.success(f"✅ Attendees detected: {', '.join(attendees)}")
